...
if [ "$1" == "--clean" ]; then
    docker run --rm --volume ${PWD}/.build/layer/sharp:/build amazonlinux:nodejs_14 /bin/bash -c "source ~/.bashrc; npm init -f -y; npm install sharp --save && npm install aws-sdk --save && npm install ipfs-http-client --save && npm install --only=prod"
fi
