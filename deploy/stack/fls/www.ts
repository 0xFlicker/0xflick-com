...
export class WwwStack extends Construct {
  ...
  constructor(scope: Construct, id: string, props: IProps) {
    ...
    const tableNamesParam = new ssm.StringParameter(this, "TableNamesParam", {
      parameterName: "FLS_WWW_DynamoDB_TableNames",
    ...
    const permissiveCachePolicy = new cloudfront.CachePolicy(
      ...
      {
        queryStringBehavior: cloudfront.CacheQueryStringBehavior.all(),
        headerBehavior: whiteListedHeaders
          ? cloudfront.CacheHeaderBehavior.allowList(...whiteListedHeaders)
          : cloudfront.CacheHeaderBehavior.none(),
        cookieBehavior: {
          behavior: whiteListedCookies?.length ? "whitelist" : "all",
          ...(whiteListedCookies?.length && { cookies: whiteListedCookies }),
        },
    ...
    const distribution = new cloudfront.Distribution(this, "www", {
      ...
      enableLogging: withLogging ? true : undefined,
      defaultRootObject: "",
      defaultBehavior: {
      ...
      additionalBehaviors: {
        ...
        "api/graphql": {
          origin: new origins.HttpOrigin(
            `${graphqlApi.apiId}.execute-api.${cdk.Aws.REGION}.${cdk.Aws.URL_SUFFIX}`,
            {
